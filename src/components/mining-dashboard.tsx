...
import { 
  ...
  Wallet, Zap, Clock, Cpu, 
  Network, Activity, Check, Rocket
} from "lucide-react";
...
function RateBreakdownDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="bg-background border-border max-w-[280px] sm:max-w-xs w-[90%] rounded-3xl shadow-2xl p-4 sm:p-6">
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
           ...
           <button 
            ...
            >
                ...
                <span className="text-[9px] font-black text-foreground uppercase tracking-widest">
                    {isSessionActive ? totalMiningRate.toFixed(2) : "0.00"} HOT/H
                </span>
            </button>
      ...
      <div className="px-2 pt-0 pb-4 sm:px-4 sm:pt-0 space-y-3 sm:space-y-4">
        <div className="relative min-h-[200px] xs:min-h-[240px] sm:min-h-[280px] w-full rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden border border-border bg-card shadow-xl">
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full px-4 xs:px-6 flex flex-col items-center">
            {/* Always visible balance at the top of hero card */}
            <div className="text-center mb-2 sm:mb-4">
                <p className="text-primary/40 text-[7px] xs:text-[8px] sm:text-[9px] font-black uppercase tracking-[0.5em] mb-0.5">Available Balance</p>
                <div className="flex items-center justify-center gap-1.5">
                    <h2 className="text-foreground text-xl xs:text-2xl sm:text-3xl font-black tracking-tighter tabular-nums">
                        {(userProfile?.minedCoins || 0).toFixed(4)}
                    </h2>
                    <span className="text-primary font-black text-[10px] sm:text-xs italic uppercase">HOT</span>
                </div>
            </div>

            {isSessionActive ? (
              <div className="flex flex-col items-center animate-in zoom-in-95 duration-500 w-full max-w-md">
                <span className="text-primary/40 text-[7px] xs:text-[8px] sm:text-[10px] font-black uppercase tracking-[0.5em] mb-1">Live Session Yield</span>
                ...
                <div className="flex items-center gap-2 xs:gap-3 sm:gap-4 bg-primary/10 px-5 py-2 rounded-full border border-primary/20 shadow-lg mb-3">
                ...
                <div className="w-full max-w-[240px] xs:max-w-[280px] sm:max-w-[320px] space-y-2">
                  <div className="relative">
                      <Progress value={sessionProgress} className="h-1.5 xs:h-2 bg-muted overflow-hidden rounded-full" />
                      <div className="absolute inset-0 h-1.5 xs:h-2 bg-gradient-to-r from-primary to-accent blur-sm opacity-50 transition-all duration-1000" style={{ width: `${sessionProgress}%` }} />
                  </div>

                  <div className="grid grid-cols-2 gap-2 sm:gap-3 pt-1">
                    <button onClick={() => setIsClaimDialogOpen(true)} className="h-12 xs:h-14 sm:h-16 rounded-xl xs:rounded-2xl bg-secondary border border-border hover:bg-accent text-foreground flex flex-col items-center justify-center gap-0.5 group transition-all shadow-sm">
                      <Coins className="h-3.5 w-3.5 xs:h-4 sm:h-5 text-amber-500 group-hover:scale-110 transition-transform" />
                      <span className="text-[7px] xs:text-[8px] sm:text-[10px] font-black uppercase tracking-widest">Claim HOT</span>
                    </button>
                    <button onClick={handleTurboBoost} disabled={!!isProcessing} className="h-12 xs:h-14 sm:h-16 rounded-xl xs:rounded-2xl bg-secondary border border-border hover:bg-accent text-foreground flex flex-col items-center justify-center gap-0.5 group disabled:opacity-50 transition-all shadow-sm">
                      {isProcessing === 'turbo' ? <Loader2 className="animate-spin h-3.5 w-3.5 xs:h-4 sm:h-5" /> : <><Zap className="h-3.5 w-3.5 xs:h-4 sm:h-5 text-primary group-hover:animate-pulse" /><span className="text-[7px] xs:text-[8px] sm:text-[10px] font-black uppercase tracking-widest">Turbo Boost</span></>}
                    </button>
                  </div>
                </div>
              </div>
            ) : (
              <div className="flex flex-col items-center animate-in fade-in duration-700 w-full max-w-sm pt-2">
                <Button onClick={handleStartMining} disabled={isStarting} className="w-full h-14 xs:h-16 rounded-2xl bg-primary hover:bg-primary/90 text-primary-foreground font-black text-base xs:text-lg sm:text-xl uppercase tracking-tighter shadow-xl active:scale-95 transition-all border-b-4 border-primary/80 active:border-b-0">
                  {isStarting ? <Loader2 className="animate-spin mr-2 h-4 w-4" /> : <Play className="mr-2 h-4 w-4 fill-current" />}
                  Initialize Node
                </Button>
                <p className="text-muted-foreground text-[7px] xs:text-[8px] sm:text-[9px] font-black tracking-[0.2em] mt-4 uppercase text-center max-w-[160px] leading-relaxed">Terminal standby: begin cycle</p>
              </div>
            )}
          </div>
        </div>
        ...
        <AlertDialog open={showBoostSuccess} onOpenChange={setShowBoostSuccess}>
          <AlertDialogContent className="bg-background border-primary/50 w-[90%] max-w-[320px] rounded-3xl">
              <AlertDialogHeader className="text-center">
                  <div className="mx-auto flex h-10 w-10 items-center justify-center rounded-full bg-primary/20 mb-4 border-2 border-primary/50">
                  ...
                  <AlertDialogDescription className="text-muted-foreground text-[10px] xs:text-xs">
                      Efficiency increased by <span className="text-foreground font-bold">+0.10 HOT/hr</span>.
                  </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter className="mt-4">
                  <AlertDialogAction onClick={() => setShowBoostSuccess(false)} className="w-full bg-primary text-primary-foreground hover:bg-primary/90 font-black uppercase tracking-widest text-[10px] h-10 sm:h-12 rounded-2xl shadow-lg">Acknowledge</AlertDialogAction>
              </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>

        <div onClick={() => setIsRateBreakdownOpen(true)} className="rounded-2xl xs:rounded-[2rem] p-4 xs:p-6 sm:p-8 relative overflow-hidden bg-card border border-border cursor-pointer transition-all hover:bg-secondary/20 active:scale-[0.99] shadow-md">
          ...
          <div className="flex justify-between items-center mb-4 sm:mb-6">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className={`w-1 h-1 xs:w-1.5 rounded-full ${isSessionActive ? 'bg-primary animate-pulse shadow-[0_0_8px_currentColor]' : 'bg-destructive'}`} />
            ...
            <div className="px-2 py-0.5 rounded-full bg-secondary border border-border">
              <span className="text-foreground/80 font-mono text-[6px] xs:text-[8px] font-bold tracking-widest uppercase">{isSessionActive ? 'RUNNING' : 'OFFLINE'}</span>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
            <div className="space-y-1">
              <p className="text-muted-foreground text-[8px] xs:text-[10px] font-bold uppercase tracking-widest leading-none">Protocol Efficiency</p>
              <div className="flex items-baseline gap-2 sm:gap-4">
                ...
                <span className="text-primary font-black text-base xs:text-lg sm:text-xl italic tracking-tighter leading-none">HOT/H</span>
              </div>
              {isSessionActive && (
                <div className="flex items-center gap-2 mt-2 pt-2 border-t border-border">
                  <Activity className="h-2.5 w-2.5 text-primary" />
                  <div className="flex flex-col">
                    <span className="text-[7px] xs:text-[8px] font-black text-muted-foreground uppercase tracking-widest leading-none mb-0.5">Current_Yield</span>
                    <span className="text-sm xs:text-base font-mono font-bold text-foreground tracking-tight leading-none">{liveCoins.toFixed(6)}</span>
                  </div>
                </div>
              )}
            </div>
            <div className="h-12 xs:h-16 sm:h-24 w-full opacity-80 md:block hidden">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData}>
                  <Line type="monotone" dataKey="value" stroke={isSessionActive ? "#a855f7" : "#333"} strokeWidth={3} dot={false} isAnimationActive={false}/>
        ...
        <div className="px-2 py-3 border-t border-border flex justify-between items-center text-muted-foreground/40">
          <div className="flex items-center gap-2 sm:gap-4">
              <div className="flex items-center gap-1"><Cpu className="h-2 w-2 sm:h-3 sm:w-3" /><span className="text-[6px] sm:text-[9px] font-bold uppercase tracking-widest">Optimized_CPU</span></div>
              <div className="flex items-center gap-1"><Network className="h-2 w-2 sm:h-3 sm:w-3" /><span className="text-[6px] sm:text-[9px] font-bold uppercase tracking-widest">Hills_Net</span></div>
          </div>
          <span className="text-[6px] sm:text-[9px] font-mono tracking-tighter opacity-50 uppercase">v4.0.12_STABLE</span>
        </div>
      </div>
    </div>
  );
