...
export function MiningDashboard() {
  ...
  return (
    <div className="min-h-screen bg-background p-3 sm:p-6 space-y-4 sm:space-y-6 pb-24">
      <div className="relative min-h-[420px] xs:min-h-[480px] w-full rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden border border-border bg-card shadow-xl">
        <div className="absolute inset-0 bg-gradient-to-br from-primary/5 via-transparent to-primary/5" />
        
        {/* Header Branding */}
        <div className="absolute top-6 left-6 xs:top-8 xs:left-8 flex items-center gap-3 xs:gap-4">
          <div className="w-10 h-10 xs:w-12 xs:h-12 rounded-xl bg-secondary border border-border flex items-center justify-center shadow-inner">
            <LayoutGrid className="text-primary h-5 w-5 xs:h-6 xs:w-6" />
          </div>
          <div>
            <h1 className="text-foreground font-black text-lg xs:text-xl leading-none tracking-tight uppercase">HIREHILLS</h1>
            <p className="text-primary text-[8px] xs:text-[10px] font-bold uppercase tracking-[0.2em] mt-1">NODE CLUSTER v4.0</p>
          </div>
        </div>
        ...
        <button 
            ...
            className="absolute top-6 xs:top-8 left-1/2 -translate-x-1/2 transition-all hover:scale-105 active:scale-95 z-10 flex items-center gap-1.5 xs:gap-2 px-3 py-1.5 xs:px-4 xs:py-2 rounded-2xl bg-secondary border border-border shadow-sm hover:border-primary/50"
        >
            <Zap className="text-primary h-3 w-3 xs:h-4 xs:w-4" />
            <span className="text-[8px] xs:text-[10px] font-black text-foreground uppercase tracking-widest">
                {isSessionActive ? totalMiningRate.toFixed(2) : "0.00"} HOT/HR
            </span>
        </button>

        {/* Wallet Link */}
        <Link href="/wallet" className="absolute top-6 xs:top-8 right-6 xs:right-8 transition-transform hover:scale-110 active:scale-95 z-10">
          <div className="w-10 h-10 xs:w-12 xs:h-12 rounded-xl bg-secondary border border-border flex items-center justify-center shadow-inner hover:bg-accent">
            <Wallet className="text-primary h-5 w-5 xs:h-6 xs:w-6" />
          </div>
        </Link>

        {/* Main Content Area */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full px-4 xs:px-6 flex flex-col items-center mt-4 xs:mt-0">
          {isSessionActive ? (
            <div className="flex flex-col items-center animate-in zoom-in-95 duration-500 w-full max-w-md">
              <span className="text-primary/40 text-[8px] xs:text-[10px] font-black uppercase tracking-[0.5em] mb-2 xs:mb-3">Live Session Counter</span>
              <div className="relative mb-3 xs:mb-4">
                <h2 className="text-foreground text-5xl xs:text-6xl sm:text-7xl md:text-8xl font-black tracking-tighter tabular-nums drop-shadow-sm text-center">
                  {liveCoins.toFixed(4)}
                </h2>
                <div className="absolute inset-0 border-y border-border/20 animate-scan-line pointer-events-none" />
              </div>
              
              <div className="flex items-center gap-2 xs:gap-3 bg-primary/10 px-3 py-1 xs:px-4 xs:py-1.5 rounded-full border border-primary/20">
                <div className="w-1.5 h-1.5 xs:w-2 xs:h-2 rounded-full bg-primary animate-pulse shadow-[0_0_10px_currentColor]" />
                <span className="text-primary text-[8px] xs:text-[10px] font-black tracking-[0.2em] xs:tracking-[0.3em] uppercase italic">Node_Sync_Active</span>
              </div>

              <div className="mt-6 xs:mt-8 w-full max-w-[280px] xs:max-w-[320px] space-y-3 xs:space-y-4">
                <div className="space-y-1.5 xs:space-y-2">
                    <div className="relative">
                        <Progress value={sessionProgress} className="h-2 xs:h-2.5 bg-muted overflow-hidden rounded-full" />
                        <div 
                            className="absolute inset-0 h-2 xs:h-2.5 bg-gradient-to-r from-primary to-accent blur-sm opacity-50 transition-all duration-1000" 
                            style={{ width: `${sessionProgress}%` }}
                        />
                    </div>
                    <div className="flex items-center justify-center gap-1.5 xs:gap-2 text-muted-foreground text-[8px] xs:text-[10px] font-black uppercase tracking-[0.1em] xs:tracking-[0.2em]">
                        <Clock className="h-2.5 w-2.5 xs:h-3 xs:w-3" />
                        <span>{timeRemaining} CYCLE REMAINING</span>
                    </div>
                </div>

                <div className="grid grid-cols-2 gap-2 xs:gap-3 pt-1 xs:pt-2">
                  <button 
                    onClick={() => setIsClaimDialogOpen(true)}
                    className="h-14 xs:h-16 rounded-2xl bg-secondary border border-border hover:bg-accent text-foreground flex flex-col items-center justify-center gap-1 group transition-all shadow-sm"
                  >
                    <Coins className="h-4 w-4 xs:h-5 xs:w-5 text-amber-500 group-hover:scale-110 transition-transform" />
                    <span className="text-[8px] xs:text-[10px] font-black uppercase tracking-widest">Claim Coins</span>
                  </button>

                  <button 
                    onClick={handleTurboBoost}
                    disabled={!!isProcessing}
                    className="h-14 xs:h-16 rounded-2xl bg-secondary border border-border hover:bg-accent text-foreground flex flex-col items-center justify-center gap-1 group disabled:opacity-50 transition-all shadow-sm"
                  >
                    {isProcessing === 'turbo' ? (
                        <Loader2 className="animate-spin h-4 w-4 xs:h-5 xs:w-5" />
                    ) : (
                        <>
                            <Zap className="h-4 w-4 xs:h-5 xs:w-5 text-primary group-hover:animate-pulse" />
                            <span className="text-[8px] xs:text-[10px] font-black uppercase tracking-widest">Turbo Boost</span>
                        </>
                    )}
                  </button>
                </div>
            ...
            <div className="flex flex-col items-center animate-in fade-in duration-700 w-full max-w-sm">
              <div className="relative mb-8 xs:mb-12">
                <span className="hot-logo-text text-8xl xs:text-9xl md:text-[10rem] opacity-10 select-none leading-none">HOT</span>
                <div className="absolute inset-0 flex items-center justify-center">
                    <Database className="h-10 w-10 xs:h-12 xs:w-12 text-foreground/5" />
                </div>
              </div>
              <Button 
                onClick={handleStartMining}
                disabled={isStarting}
                className="w-full h-16 xs:h-20 rounded-[1.5rem] xs:rounded-[2rem] bg-primary hover:bg-primary/90 text-primary-foreground font-black text-lg xs:text-xl uppercase tracking-tighter shadow-xl active:scale-95 transition-all border-b-4 border-primary/80 active:border-b-0"
              >
                {isStarting ? <Loader2 className="animate-spin mr-2 xs:mr-3 h-5 w-5 xs:h-6 xs:w-6" /> : <Play className="mr-2 xs:mr-3 h-5 w-5 xs:h-6 xs:w-6 fill-current" />}
                Initialize Node
              </Button>
              <p className="text-muted-foreground text-[8px] xs:text-[9px] font-black tracking-[0.2em] xs:tracking-[0.4em] mt-6 xs:mt-8 uppercase text-center max-w-[180px] xs:max-w-[200px] leading-relaxed">
      ...
      <AlertDialog open={showBoostSuccess} onOpenChange={setShowBoostSuccess}>
        <AlertDialogContent className="bg-background border-primary/50 w-[90%] max-w-[320px] rounded-3xl">
            <AlertDialogHeader className="text-center">
                <div className="mx-auto flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-full bg-primary/20 mb-4 border-2 border-primary/50">
                    <Rocket className="h-6 w-6 sm:h-8 sm:w-8 text-primary" />
                </div>
                <AlertDialogTitle className="text-lg sm:text-xl font-black text-primary uppercase italic leading-none">TURBO_ACTIVE</AlertDialogTitle>
                <AlertDialogDescription className="text-muted-foreground text-xs sm:text-sm">
      ...
      <div 
        onClick={() => setIsRateBreakdownOpen(true)}
        className="rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-8 relative overflow-hidden bg-card border border-border cursor-pointer transition-all hover:bg-secondary/20 active:scale-[0.99] shadow-md"
      >
        ...
        <div className="flex justify-between items-center mb-6 sm:mb-8">
          <div className="flex items-center gap-2 xs:gap-3">
            <div className={`w-1.5 h-1.5 xs:w-2 xs:h-2 rounded-full ${isSessionActive ? 'bg-primary animate-pulse shadow-[0_0_8px_currentColor]' : 'bg-destructive'}`} />
            <span className="text-foreground font-black text-base xs:text-lg uppercase tracking-tighter italic">Computation_Core</span>
            <ChevronDown className="h-4 w-4 xs:h-5 xs:w-5 text-muted-foreground/20" />
          </div>
          <div className="px-3 py-1 xs:px-4 xs:py-1.5 rounded-full bg-secondary border border-border">
            <span className="text-foreground/80 font-mono text-[8px] xs:text-[10px] font-bold tracking-widest uppercase">
              {isSessionActive ? 'RUNNING' : 'OFFLINE'}
            </span>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 xs:gap-8 items-end">
          <div className="space-y-1 xs:space-y-2">
            <p className="text-muted-foreground text-[10px] xs:text-xs font-bold uppercase tracking-widest leading-none">Protocol Efficiency</p>
            <div className="flex items-baseline gap-2 xs:gap-4">
              <h2 className="text-foreground text-4xl xs:text-5xl font-black tracking-tighter italic tabular-nums leading-none">
                {isSessionActive ? totalMiningRate.toFixed(2) : "0.00"}
              </h2>
              <span className="text-primary font-black text-lg xs:text-xl italic tracking-tighter leading-none">HOT/HR</span>
            </div>
            {isSessionActive && (
              <div className="flex items-center gap-2 mt-3 xs:mt-4 pt-3 xs:pt-4 border-t border-border">
                <Activity className="h-3 w-3 xs:h-4 xs:w-4 text-primary" />
                <div className="flex flex-col">
                  <span className="text-[8px] xs:text-[9px] font-black text-muted-foreground uppercase tracking-widest leading-none mb-1">Current_Yield</span>
                  <span className="text-base xs:text-lg font-mono font-bold text-foreground tracking-tight leading-none">{liveCoins.toFixed(6)}</span>
                </div>
              </div>
            )}
          </div>
          
          <div className="h-16 xs:h-24 w-full opacity-80 md:block hidden">
      ...
      {/* Footer Meta */}
      <div className="px-2 xs:px-4 py-4 xs:py-6 border-t border-border flex justify-between items-center text-muted-foreground/40">
        <div className="flex items-center gap-3 xs:gap-4">
            <div className="flex items-center gap-1.5 xs:gap-2">
                <Cpu className="h-2.5 w-2.5 xs:h-3 xs:w-3" />
                <span className="text-[7px] xs:text-[9px] font-bold uppercase tracking-widest">Optimized_CPU</span>
            </div>
            <div className="flex items-center gap-1.5 xs:gap-2">
                <Network className="h-2.5 w-2.5 xs:h-3 xs:w-3" />
                <span className="text-[7px] xs:text-[9px] font-bold uppercase tracking-widest">Hills_Net</span>
            </div>
        </div>
        <span className="text-[7px] xs:text-[9px] font-mono tracking-tighter opacity-50 uppercase">v4.0.12_STABLE</span>
      </div>
    </div>
  );
}