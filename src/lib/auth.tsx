...
interface AuthContextType {
  ...
  enrollAllEligibleUsers: (tournamentId: string) => Promise<void>;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    calculateLiveValues();
    interval = setInterval(calculateLiveValues, 100); // 100ms for continuous increase feel
...
const enrollAllEligibleUsers = useCallback(async (tournamentId: string) => {
    if (!userProfile?.isAdmin) {
      toast({ title: 'Unauthorized', description: 'You are not an admin.', variant: 'destructive' });
      throw new Error("User is not an admin.");
    }
    ...
    try {
      const functions = getFunctions();
      const enrollUsersFn = httpsCallable(functions, 'enrollUsersInTournament');
      const result = await enrollUsersFn({ tournamentId });
      const data = result.data as { success: boolean, message?: string, count?: number };
      
      if (data.success) {
        toast({ title: 'Enrolled Users', description: `Successfully enrolled ${data.count || 0} eligible users.` });
      } else {
        throw new Error(data.message || 'Failed to enroll users.');
      }
    } catch (error: any) {
      console.error("Error enrolling users in tournament:", error);
